# Akorede Adetola

<div class="about-panel">
  <div class="about-photo">
    <img src="images/photo.svg" alt="Portrait of Your Name">
    <div class="sweep"></div>
    <div class="about-photo__caption"><span>FIG.01</span><b>PORTRAIT</b></div>
  </div>
  <div class="about-body">
    <p class="lead"><strong>Researching on the engineering fundamentals for the next kardashev climb </strong> working at the intersection of Intelligent Vision Systems and Automation.</p>
    <p>I explore how to make current vision sytems use the inductive bias present in their world view.</p>
    <p>Currently: ML Researcher <em>Sage Engineering Lab</em>.  Mechanical Engineering <em>Olabisi Onabanjo University</em>. See the <a href="#cv">CV</a> for the full record.</p>
    <div class="contact-strip">
      <a class="btn btn--email" href="mailto:adetola11korede@proton.me"><svg><use href="#i-mail"/></svg> Email</a>
      <a class="btn btn--accent" href="https://scholar.google.com" target="_blank" rel="noopener">Scholar</a>
      <a class="btn" href="https://github.com/Akoredetimothy" target="_blank" rel="noopener"><svg><use href="#i-code"/></svg> GitHub</a>
      <a class="btn" href="https://orcid.org" target="_blank" rel="noopener">ORCID</a>
    </div>
  </div>
</div>

## Navigate
<div class="tilegrid">
  <a class="tile tile--a1" href="#books">
    <svg class="tile__icon"><use href="#i-book"/></svg>
    <span class="tile__label" style="font-weight: 900; font-size: 1.4rem; color: #ffffff;">Books</span>
  </a>
  <a class="tile tile--live" href="#graphs">
    <div class="tile__face tile__face--front">
      <svg class="tile__icon"><use href="#i-graph"/></svg>
      <span class="tile__label" style="font-weight: 900; font-size: 1.4rem; color: #ffffff;">Graphs</span>
    </div>
    <div class="tile__face tile__face--back">
      <span class="tile__label" style="font-weight: 900; font-size: 1.4rem; color: #ffffff;">Interactive Graphs</span>
      <span class="tile__sub" style="font-weight: 900; font-size: 1.2rem; color: #ffffff;">tap to view</span>
    </div>
  </a>
  <a class="tile tile--a3" href="#gallery">
    <svg class="tile__icon"><use href="#i-gallery"/></svg>
    <span class="tile__label" style="font-weight: 900; font-size: 1.4rem; color: #ffffff;">Gallery</span>
  </a>
  <a class="tile tile--a4" href="#games">
    <svg class="tile__icon"><use href="#i-game"/></svg>
    <span class="tile__label" style="font-weight: 900; font-size: 1.4rem; color: #ffffff;">Games</span>
  </a>
  <a class="tile tile--a2" href="#mix">
    <svg class="tile__icon"><use href="#i-mix"/></svg>
    <span class="tile__label" style="font-weight: 900; font-size: 1.4rem; color: #ffffff;">Mix</span>
  </a>
</div>

## Research Interests

- **Scientific machine learning** — physics-informed networks, neural operators, and uncertainty quantification
- **Computational modelling** — multiscale simulation of complex physical systems
- **Open science** — reproducible pipelines, shared benchmarks, and community datasets

## Current Focus

> Building fast surrogate models that respect physical conservation laws — and writing openly about the process on the blog.

## Quick Links

<ul class="list">
  <li><a class="list-item" href="#publications">
    <svg class="list-item__icon"><use href="#i-book"/></svg>
    <div><div class="list-item__title">Publications</div><div class="list-item__desc">Papers, preprints and talks</div></div>
    <svg class="list-item__chevron"><use href="#i-chevron"/></svg>
  </a></li>
  <li><a class="list-item" href="#blog">
    <svg class="list-item__icon"><use href="#i-pen"/></svg>
    <div><div class="list-item__title">Blog</div><div class="list-item__desc">Notes from the research notebook</div></div>
    <svg class="list-item__chevron"><use href="#i-chevron"/></svg>
  </a></li>
  <li><a class="list-item" href="#cv">
    <svg class="list-item__icon"><use href="#i-doc"/></svg>
    <div><div class="list-item__title">Curriculum Vitae</div><div class="list-item__desc">Education, experience and service</div></div>
    <svg class="list-item__chevron"><use href="#i-chevron"/></svg>
  </a></li>
</ul>
